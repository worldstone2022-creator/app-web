...
<li>
    <div class="tw-flex tw-items-center">
        ...
        <a href="{{ route('base-legal.sources.index') }}" class="tw-ml-4 tw-text-sm tw-font-medium tw-text-gray-500 hover:tw-text-gray-700">Sources</a>
    </div>
</li>
<li>
    <div class="tw-flex tw-items-center">
        ...
        <span class="tw-ml-4 tw-text-sm tw-font-medium tw-text-gray-500 tw-truncate">{{ $source->nom }}</span>
    </div>
</li>
 </ul>
 <div class="tw-flex tw-space-x-3 mb-4">
    ...
    <a href="{{ route('base_documentaire_documents_create_form') }}?source={{ $source->id }}" 
...
<div class="tw-space-y-6">
            ...
            <div class="tw-flex tw-items-center">
                @php
                    $typeIcons = [
                        ...
                        'decret' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
                        'convention_collective' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z',
                        'jurisprudence' => 'M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3',
                        'arrete' => 'M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z',
                        'circulaire' => 'M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2'
                    ];
                    $typeColors = [
                    ...
                    $iconPath = $typeIcons[$source->type] ?? 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z';
                    $colorClass = $typeColors[$source->type] ?? 'tw-bg-gray-100 tw-text-gray-800';
                @endphp
                
                <div class="tw-h-12 tw-w-12 tw-rounded-full tw-bg-gray-100 tw-flex tw-items-center tw-justify-center tw-mr-4">
                    <svg class="tw-h-8 tw-w-8 tw-text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $iconPath }}" />
                    </svg>
                </div>
                <div>
                    <h1 class="tw-text-2xl tw-font-bold tw-text-gray-900">{{ $source->nom }}</h1>
                    <div class="tw-mt-1 tw-flex tw-items-center tw-space-x-2">
                        <span class="tw-inline-flex tw-items-center tw-px-2.5 tw-py-0.5 tw-rounded-full tw-text-sm tw-font-medium {{ $colorClass }}">
                            {{ $source->type_libelle }}
                        </span>
                        <span class="tw-text-sm tw-text-gray-500">•</span>
                        <span class="tw-text-sm tw-text-gray-500">{{ isset($documents) ? $documents->total() : $source->documents->count() }} document(s)</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="tw-border-t tw-border-gray-200 tw-px-4 tw-py-5 sm:tw-p-0">
            <dl class="sm:tw-divide-y sm:tw-divide-gray-200">
                <div class="tw-py-4 sm:tw-py-5 sm:tw-grid sm:tw-grid-cols-3 sm:tw-gap-4 sm:tw-px-6">
                    <dt class="tw-text-sm tw-font-medium tw-text-gray-500">Nom de la source</dt>
                    <dd class="tw-mt-1 tw-text-sm tw-text-gray-900 sm:tw-mt-0 sm:tw-col-span-2">{{ $source->nom }}</dd>
                </div>

                <div class="tw-py-4 sm:tw-py-5 sm:tw-grid sm:tw-grid-cols-3 sm:tw-gap-4 sm:tw-px-6">
                    <dt class="tw-text-sm tw-font-medium tw-text-gray-500">Type de source</dt>
                    <dd class="tw-mt-1 tw-text-sm tw-text-gray-900 sm:tw-mt-0 sm:tw-col-span-2">
                        <div class="tw-flex tw-items-center">
                            <span class="tw-inline-flex tw-items-center tw-px-2.5 tw-py-0.5 tw-rounded-full tw-text-xs tw-font-medium {{ $colorClass }}">
                                {{ $source->type_libelle }}
                            </span>
                            <span class="tw-ml-2 tw-text-sm tw-text-gray-600">
                                @switch($source->type)
                                    @case('loi')
                                        ...
                                        @break
                                    @case('decret')
                                        Texte pris par le pouvoir exécutif
                                        @break
                                    @case('convention_collective')
                                        Accord entre partenaires sociaux
                                        @break
                                    @case('jurisprudence')
                                        Décision de justice de référence
                                        @break
                                    @case('arrete')
                ...
                <div class="tw-py-4 sm:tw-py-5 sm:tw-grid sm:tw-grid-cols-3 sm:tw-gap-4 sm:tw-px-6">
                    ...
                    <dd class="tw-mt-1 tw-text-sm tw-text-gray-900 sm:tw-mt-0 sm:tw-col-span-2">
                        <span class="tw-inline-flex tw-items-center tw-px-2.5 tw-py-0.5 tw-rounded-full tw-text-xs tw-font-medium tw-bg-green-100 tw-text-green-800">
                            {{ isset($documents) ? $documents->total() : $source->documents->count() }} document(s) associé(s)
                        </span>
                    </dd>
                </div>
                ...
                <div class="tw-py-4 sm:tw-py-5 sm:tw-grid sm:tw-grid-cols-3 sm:tw-gap-4 sm:tw-px-6">
                    ...
                    <dd class="tw-mt-1 tw-text-sm tw-text-gray-900 sm:tw-mt-0 sm:tw-col-span-2">
                        <div class="tw-space-y-1">
                            <div>Créé le : {{ $source->created_at->format('d/m/Y à H:i') }}</div>
                            <div>Modifié le : {{ $source->updated_at->format('d/m/Y à H:i') }}</div>
                        </div>
                    </dd>
                </div>

                <div class="tw-py-4 sm:tw-py-5 sm:tw-grid sm:tw-grid-cols-3 sm:tw-gap-4 sm:tw-px-6">
                    <dt class="tw-text-sm tw-font-medium tw-text-gray-500">Consultation publique</dt>
                    <dd class="tw-mt-1 tw-text-sm tw-text-gray-900 sm:tw-mt-0 sm:tw-col-span-2">
                        <a href="{{ route('baselegal_consultation', ['source_type' => $source->type]) }}" 
                           target="_blank"
                           class="tw-text-gray-600 hover:tw-text-gray-500">
                            Voir tous les documents de ce type
                            <svg class="tw-inline tw-h-4 tw-w-4 tw-ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @if(isset($documents) && $documents->total() > 0)
        <div class="tw-bg-white tw-shadow tw-rounded-lg">
            <div class="tw-px-4 tw-py-5 sm:tw-p-6">
                ...
                <form method="GET" action="{{ route('base-legal.sources.show', $source) }}" class="tw-space-y-4">
    ...
    <div class="tw-bg-white tw-shadow tw-rounded-lg">
                            ...
                            <div class="tw-flex-1 tw-min-w-0">
                                ...
                                @if($document->thematiques->count() > 0)
                                    <div class="tw-mt-2 tw-flex tw-flex-wrap tw-gap-1">
                                        <span class="tw-text-xs tw-text-gray-500 tw-mr-2">Thématiques :</span>
        ...
        @else
            <!-- Aucun document pour cette source -->
            <div class="tw-text-center tw-py-12">
                <svg class="tw-mx-auto tw-h-12 tw-w-12 tw-text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <p class="tw-mt-1 tw-text-sm tw-text-gray-500">
                    Aucun document n'est associé à cette source pour le moment.
                </p>
                <div class="tw-mt-6">
                    <a href="{{ route('base_documentaire_documents_create_form') }}?source={{ $source->id }}" 
                       class="tw-inline-flex tw-items-center tw-px-4 tw-py-2 tw-border tw-border-transparent tw-shadow-sm tw-text-sm tw-font-medium tw-rounded-md tw-text-white tw-bg-gray-600 hover:tw-bg-gray-700">
    ...
    @if(isset($documents) && $documents->total() > 0)
        <div class="tw-bg-white tw-shadow tw-rounded-lg">
            <div class="tw-px-4 tw-py-5 sm:tw-p-6">
                <h3 class="tw-text-lg tw-font-medium tw-leading-6 tw-text-gray-900 tw-mb-4">Statistiques des documents</h3>
                <div class="tw-grid tw-grid-cols-1 tw-gap-4 sm:tw-grid-cols-2 lg:tw-grid-cols-4">
                    ...
                    <div class="tw-bg-gray-50 tw-rounded-lg tw-p-4">
                        <div class="tw-flex tw-items-center">
                            <div class="tw-flex-shrink-0">
                                <svg class="tw-h-6 tw-w-6 tw-text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                            </div>
                            <div class="tw-ml-3">
                                <p class="tw-text-sm tw-font-medium tw-text-gray-500">Total documents</p>
                                <p class="tw-text-lg tw-font-semibold tw-text-gray-900">{{ $documents->total() }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tw-bg-green-50 tw-rounded-lg tw-p-4">
                        <div class="tw-flex tw-items-center">
                            ...
                            <div class="tw-ml-3">
                                <p class="tw-text-sm tw-font-medium tw-text-gray-500">Documents actifs</p>
                                <p class="tw-text-lg tw-font-semibold tw-text-gray-900">{{ $documents->where('actif', true)->count() }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tw-bg-blue-50 tw-rounded-lg tw-p-4">
                        <div class="tw-flex tw-items-center">
                            <div class="tw-flex-shrink-0">
                                <svg class="tw-h-6 tw-w-6 tw-text-blue-400" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clip-rule="evenodd" />
                                </svg>
                            </div>
                            <div class="tw-ml-3">
                                <p class="tw-text-sm tw-font-medium tw-text-gray-500">Avec fichier PDF</p>
                                <p class="tw-text-lg tw-font-semibold tw-text-gray-900">{{ $documents->whereNotNull('fichier_pdf')->count() }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tw-bg-purple-50 tw-rounded-lg tw-p-4">
                        <div class="tw-flex tw-items-center">
                            <div class="tw-flex-shrink-0">
                                <svg class="tw-h-6 tw-w-6 tw-text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                                </svg>
                            </div>
                            <div class="tw-ml-3">
                                <p class="tw-text-sm tw-font-medium tw-text-gray-500">Thématiques liées</p>
                                <p class="tw-text-lg tw-font-semibold tw-text-gray-900">
                                    {{ $documents->pluck('thematiques')->flatten()->unique('id')->count() }}
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    @php
        $autresSourcesMemeType = \App\Models\Source::where('type', $source->type)
            ->where('id', '!=', $source->id)
            ->withCount('documents')
            ->take(5)
            ->get();
    ...
    @if($autresSourcesMemeType->count() > 0)
        <div class="tw-bg-white tw-shadow tw-rounded-lg">
            <div class="tw-px-4 tw-py-5 sm:tw-p-6">
                <h3 class="tw-text-lg tw-font-medium tw-leading-6 tw-text-gray-900 tw-mb-4">
                    Autres sources de type "{{ $source->type_libelle }}"
                </h3>
                <div class="tw-space-y-3">
                    @foreach($autresSourcesMemeType as $autreSource)
                        <div class="tw-flex tw-items-center tw-justify-between tw-p-3 tw-border tw-border-gray-200 tw-rounded-lg hover:tw-bg-gray-50">
                            <div class="tw-flex-1 tw-min-w-0">
                                <h4 class="tw-text-sm tw-font-medium tw-text-gray-900 tw-truncate">
                                    <a href="{{ route('base-legal.sources.show', $autreSource) }}" 
                                       class="hover:tw-text-gray-600">
                                        {{ $autreSource->nom }}
                                    </a>
                                </h4>
                                <p class="tw-text-xs tw-text-gray-500 tw-mt-1">
                                    Créé le {{ $autreSource->created_at->format('d/m/Y') }}
                                </p>
                            </div>
                            <div class="tw-flex-shrink-0 tw-flex tw-items-center tw-space-x-2">
                                <span class="tw-inline-flex tw-items-center tw-px-2.5 tw-py-0.5 tw-rounded-full tw-text-xs tw-font-medium tw-bg-gray-100 tw-text-gray-800">
                                    {{ $autreSource->documents_count }} doc(s)
                                </span>
                                <a href="{{ route('base-legal.sources.show', $autreSource) }}" 
        ...
        <div class="tw-px-4 tw-py-5 sm:tw-p-6">
            <div class="tw-flex tw-justify-between tw-items-center">
                ...
                <div class="tw-flex tw-space-x-3">
                    ...
                    <form action="{{ route('base-legal.sources.destroy', $source) }}" method="POST" class="tw-inline-block">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
                                onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette source ?\n\nAttention: Cette action supprimera également tous les documents associés ({{ isset($documents) ? $documents->total() : $source->documents->count() }} document(s)).')"
